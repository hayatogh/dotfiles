...
if !s:settings.use_css
  ...
  function! s:HtmlOpening(id, extra_attrs)
    let a = ""
    if synIDattr(a:id, "inverse", s:whatterm)
    ...
    if synIDattr(a:id, "bold", s:whatterm) | let a = a . "<b>" | endif
    if synIDattr(a:id, "italic", s:whatterm) | let a = a . "<i>" | endif
    if synIDattr(a:id, "underline", s:whatterm) | let a = a . "<u>" | endif
    return a
  endfun
...
function! s:CSS1(id)
  ...
  if synIDattr(a:id, "bold", s:whatterm) | let a = a . "font-weight: bold; " | endif
  if synIDattr(a:id, "italic", s:whatterm) | let a = a . "font-style: italic; " | endif
  if synIDattr(a:id, "underline", s:whatterm) | let a = a . "text-decoration: underline; " | endif
  return a
endfun
...
if !exists('g:toimage_additionalCss') | let g:toimage_additionalCss = [''] | endif
call extend(s:lines, [
		\ "<style type=\"text/css\">",
		\ s:settings.use_xhtml ? "" : "<!--"]
		\ + g:toimage_additionalCss
		\ + [s:settings.use_xhtml ? "" : '-->',
		\ "</style>"
		\ ])
...
if s:settings.use_css
  ...
  if !exists('g:toimage_font_size') | let g:toimage_font_size = '1em' | endif
  call append('.', '* { font-size: ' . g:toimage_font_size . '; }')
  +
...
if !exists('g:toimage_no_hyperlinks')
